// ════════════════════════════════════════════════════════════════
//  Mei HaGalil GIS — Export Feature (standalone module)
...
function injectUI() {
  ...
  modals.innerHTML = ''+
    ...
    '<div class="exp-bg" id="exp-fmt-modal" onclick="if(event.target.id===\'exp-fmt-modal\')closeFmtModal()">'+
      '<div class="exp-mod">'+
        '<div class="exp-t">📦 מצא נתונים — בחר פורמט יצוא</div>'+
        '<div class="exp-stats">'+
        ...
        '</div>'+
        '<div class="exp-fmt">'+
          '<div class="exp-fmt-c sel" data-fmt="geojson" onclick="window.expSelectFmt(\'geojson\')"><div class="exp-fmt-i">🗺️</div><div class="exp-fmt-n">GeoJSON</div><div class="exp-fmt-d">סטנדרט GIS</div></div>'+
          '<div class="exp-fmt-c" data-fmt="dxf" onclick="window.expSelectFmt(\'dxf\')"><div class="exp-fmt-i">📐</div><div class="exp-fmt-n">DXF</div><div class="exp-fmt-d">AutoCAD</div></div>'+
          '<div class="exp-fmt-c" data-fmt="csv" onclick="window.expSelectFmt(\'csv\')"><div class="exp-fmt-i">📊</div><div class="exp-fmt-n">CSV</div><div class="exp-fmt-d">Excel</div></div>'+
        '</div>'+
        '<div class="exp-btns">'+
          '<button class="exp-btn exp-btn-p" onclick="window.expGenerate()">⚙️ צור קובץ</button>'+
    ...
    '<div class="exp-bg" id="exp-done-modal" onclick="if(event.target.id===\'exp-done-modal\')closeDoneModal()">'+
      '<div class="exp-mod">'+
        ...
        '</div>'+
        '<div class="exp-warn">💡 שליחה במייל פותחת את לקוח המייל שלך. תצטרך לצרף את הקובץ ידנית אחרי הורדה (מגבלת דפדפן).</div>'+
        '<div class="exp-btns">'+
...
function closeFmtModal() { document.getElementById('exp-fmt-modal').classList.remove('open'); }
...
function renderCatsList() {
  // Aggregate feature counts per category across all loaded villages
  var counts = {};
  ...
  if (!keys.length) {
    document.getElementById('exp-cats').innerHTML = '<div style="grid-column:span 2;text-align:center;padding:20px;color:#64748b">אין שכבות טעונות. העלה קבצים תחילה.</div>';
    return;
  }
...
async function filterAndShowFmt() {
  ...
  // Re-fetch all villages' GeoJSON to get raw features with properties
  gFiltered = [];
...
window.expGenerate = function() {
  ...
  // BOM for CSV (Excel Hebrew support)
  if (gExportFormat === 'csv') content = '\uFEFF' + content;

  gExportBlob = new Blob([content], { type: mime });
...
// ── DXF GENERATION (minimal AutoCAD R12 ASCII) ──
...
window.expEmail = function() {
  ...
  // Trigger download first (so user has the file ready to attach)
  window.expDownload();
